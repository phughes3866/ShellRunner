...
class argChecker():
    ...
    def __init__(self):
        self.currentCommand = "<cmd not yet defined: pre-processing>"

    def reportArgError(self, errorStr):
        contextStr = "\n\nOffending command:: {}".format(self.currentCommand)
    ...
    def sanitiseSpawnCmdArgs(self):
        ...
        if not runcmd or not isinstance(runcmd, str):
            ...
            # build a dictionary of substitution variables that can be expanded in our command
            ...
            # split command into shell tokens (for subprocess) BEFORE performing var substitution on tokens
        ...
        self.currentCommand = truncateStr(runcmd, length=200)
        self.cmdArgs["cmd200Str"] = self.currentCommand
        self.cmdArgs["cmd50Str"] = truncateStr(self.currentCommand, length=50)
...
class runShellCommand(Thread):
    ...
    def __init__(self, passedArgs, targetView={}):
        ...
        self.db = DebugLogger(self.cmdArgs["cmd50Str"], prefix="ShellRunner Run")
            ...
            self.errStr = ("No text processed due to shell command timeout after {} seconds.\n\n"
                            "The timeout can be set in the range 1-600 seconds.\n\n"
                              "Offending command:: {}".format(self.cmdArgs["textCmdTimeout"],
                                                              self.cmdArgs["cmd50Str"]))
            return
        except Exception as err:
            self.errStr = ("Shell command failed as an unexpected exception occurred.\n\n"
                              "Offending command:: {}\n\n"
                              "Details:: {}\n\n{}".format(self.cmdArgs["cmd50Str"], err.__class__.__name__, err))
            return
...
class WindowShellrunnerTextCommandCommand(sublime_plugin.WindowCommand, argChecker):
    ...
    def run(self, **kwargs):
        ...
        doTxtCommand.start()
        status_message = "ShellRunner running: {}".format(self.cmdArgs["cmd50Str"])
